...
impl RuntimeManager {
    pub fn new() -> Result<Self, io::Error> {
        ...
        // FIX: Cap the worker thread count to a maximum of 8. 
        // This prevents hitting OS limits on high-core-count machines 
        // or restrictive environments, which causes silent panics.
        const MAX_WORKERS: usize = 8;
        let worker_count = num_cores.min(MAX_WORKERS).max(1);

        info!(
            "Tokio Runtime initializing with {} worker threads (Capped from {} cores, max {}).", 
            worker_count, 
            num_cores, 
            MAX_WORKERS
        );
        ...
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .worker_threads(worker_count) // Use the capped, safer count
            .enable_all()
            .build()
            // Improve the error map to return a better message if it fails gracefully
