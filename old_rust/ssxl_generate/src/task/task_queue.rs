...
use std::sync::Arc;
// Restored necessary imports for handle_chunk_unit
...
// FIX 1: Publicly re-export both items to fix all external dependency errors.
pub use ssxl_shared::message::generation_message::{GenerationMessage, GenerationTask};

// FIX 2: REMOVE THE GLOB IMPORT (use ssxl_shared::message::generation_message::GenerationTask::*;)
// It causes the E0432 error and is not strictly necessary.

type DynGenerator = Box<dyn Generator + Send + Sync>;
pub const CHUNK_SIZE: i64 = SHARED_CHUNK_SIZE as i64;

// ... (handle_chunk_unit function remains unchanged) ...
...
pub fn start_request_loop(
    ...
    rt_handle.spawn(async move {
        ...
        let initial_generators_clone = generators;
        let initial_cache_clone = chunk_cache;
        let initial_state_clone = conductor_state;

        while let Some(task) = request_rx.recv().await {
            if !initial_state_clone.as_ref().is_active() {
                warn!(?task, "Dropping task — Conductor not active");
                continue;
            }
            
            // FIX 3: Use the fully qualified name (GenerationTask::VariantName)
            // This is the most stable way to handle re-exported types with variants.
            match task {
                GenerationTask::MapBuild { config, active_generator_id } => { 
                    info!("Received MapBuild command. Spawning batch task.");

                    spawn_batch_generation_task(
                        &rt_handle,
                        initial_generators_clone.as_ref().clone(),
                        initial_cache_clone.clone(),
                        active_generator_id,
                        progress_tx.clone(),
                        initial_state_clone.clone(),
                        config, 
                    );
                }

                GenerationTask::SingleChunk { chunk_coords, generator_id } => { 
                    let progress_tx = progress_tx.clone();
                    let generators = initial_generators_clone.clone();
                    let cache = initial_cache_clone.clone();
                    let state = initial_state_clone.clone();
                ...
                _ => {
                    warn!(?task, "Received unhandled GenerationTask variant. Task dropped.");
                }
            }
        }
