...
param(
    # FIX 1: Target only the SSXL_engine_tester directory.
    [string[]]$TargetDirs = @(".\SSXL_engine_tester"),
    # FIX 2: Target only GDScript files.
    [string[]]$TargetExts = @("gd"),
    # The output filename, now prepended with the directory structure.
    [string]$OutputBaseDir = "loc_reports",
    [string]$OutputFile = "ssxl_loc_report_godot_{0}.txt" -f (Get-Random)
)
