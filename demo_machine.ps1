...
public delegate int AddOneDelegate(int x);
"@
...
$mem  = [Native]::VirtualAlloc([IntPtr]::Zero, $size,
    [Native]::MEM_COMMIT -bor [Native]::MEM_RESERVE,
    [Native]::PAGE_EXECUTE_READWRITE)
if ($mem -eq [IntPtr]::Zero) { throw "VirtualAlloc failed." }
[Native]::RtlMoveMemory($mem, $code, $size)

# Create delegate for the machine code
